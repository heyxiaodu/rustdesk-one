...
use iroh::{Endpoint, EndpointAddr, EndpointId, RelayMap, RelayMode, SecretKey, TransportAddr, endpoint::presets};
use std::io;
...
pub struct IrohConfig {
    ...
    /// 自建 relay 的访问令牌（对应服务端 `access.shared_token`）。
    ///
    /// 非空时对所有 relay 条目调用 `RelayMap::with_auth_token`（iroh 1.2 在
    /// WebSocket 升级时以 `Authorization: Bearer <token>` 头发送）。为空则不注入，
    /// 行为与现状完全一致（NervDesk 改动：不改变「不配 = 无令牌」的语义）。
...
/// 解析 relay 访问令牌（选项 `iroh-relay-token`，与 `iroh-relay` 同源风格）。
///
/// 空字符串 / 纯空白 → `None`（不注入令牌，行为同现状）；否则返回 trim 后的令牌。
/// 令牌不拆分（不像 relay URL 列表那样支持逗号分隔）——它是单值密钥。
...
pub fn config_from_options() -> ResultType<IrohConfig> {
    ...
    Ok(IrohConfig {
        ...
        relay_auth_token: parse_relay_token(&crate::config::Config::get_option("iroh-relay-token")),
...
/// 由配置构造 `RelayMode`（纯函数，便于单元测试）。
///
/// - relay 列表为空 → [`RelayMode::Disabled`]（纯直连，行为同现状）；
/// - 非空且带 `relay_auth_token` → `RelayMode::Custom(map)`，其中 map 经过
///   [`RelayMap::with_auth_token`] 注入令牌（Bearer 头）；
/// - 非空且无令牌 → `Custom` 不带令牌（与现状 `RelayMode::custom(urls)` 等价）。
pub fn relay_mode_from_config(config: &IrohConfig) -> ResultType<RelayMode> {
...
#[cfg(test)]
mod tests {
    /// relay 令牌解析：空 / 纯空白 → None；有值 → trim 后的令牌；不做逗号拆分。
    #[test]
    fn parse_relay_token_handles_empty_and_whitespace() {
        assert!(parse_relay_token("").is_none());
        assert!(parse_relay_token("   ").is_none());
        assert!(parse_relay_token("\n\t").is_none());
        assert_eq!(parse_relay_token("tok-123"), Some("tok-123".to_owned()));
        assert_eq!(parse_relay_token("  tok-123  "), Some("tok-123".to_owned()));
        // 令牌是单值：逗号属于令牌本身，不该被拆开（与 relay URL 列表的逗号语法不同）
        assert_eq!(parse_relay_token("a,b"), Some("a,b".to_owned()));
    }

    /// 令牌注入：有令牌时 `RelayMap` 每个条目的 `auth_token` 被设置；
    /// 无令牌时不注入；空列表 → `Disabled`。
    #[test]
    fn relay_mode_injects_token_only_when_provided() {
        use iroh::RelayUrl;

        // 空列表 → Disabled（行为同现状，不加令牌也不报错）
        let cfg = IrohConfig {
            relay_urls: Vec::new(),
            relay_auth_token: Some("tok".to_owned()),
            ..Default::default()
        };
        assert!(matches!(relay_mode_from_config(&cfg).unwrap(), RelayMode::Disabled));

        // 无令牌 → Custom 且 auth_token 为 None（与既有 RelayMode::custom 等价）
        let cfg = IrohConfig {
            relay_urls: vec!["https://relay.example".to_owned()],
            relay_auth_token: None,
            ..Default::default()
        };
        match relay_mode_from_config(&cfg).unwrap() {
            RelayMode::Custom(map) => {
                let url: RelayUrl = "https://relay.example".parse().unwrap();
                let cfg_entry = map.get(&url).expect("relay 条目应在");
                assert!(cfg_entry.auth_token.is_none(), "未配令牌时不应注入");
            }
            other => panic!("期望 Custom，实际 {other:?}"),
        }

        // 有令牌 → Custom 且每个条目的 auth_token 都已设置
        let cfg = IrohConfig {
            relay_urls: vec!["https://relay.example".to_owned(), "https://relay2.example".to_owned()],
            relay_auth_token: Some("tok-abc".to_owned()),
            ..Default::default()
        };
        match relay_mode_from_config(&cfg).unwrap() {
            RelayMode::Custom(map) => {
                let entries = map.relays::<Vec<_>>();
                assert_eq!(entries.len(), 2, "两个 relay 都在 map 里");
                for cfg_entry in entries {
                    assert_eq!(
                        cfg_entry.auth_token.as_deref(),
                        Some("tok-abc"),
                        "令牌必须注入每个 relay 条目"
                    );
                }
