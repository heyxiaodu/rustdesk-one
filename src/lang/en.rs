...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("transport-mode-tip", "Which transport a direct connection may use. Auto tries QUIC first and falls back to the legacy KCP path. QUIC only uses QUIC, and reports the failure instead of falling back, so it can prove which transport a session used. TCP never uses QUIC. Relayed connections are unaffected."),
        ("hide-tray-tip", "Allow hiding the tray icon only if accepting sessions via a password and using the permanent password"),
    ].iter().cloned().collect();
}
